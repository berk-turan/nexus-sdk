...
mod tests {
    ...
    fn acquire_env_lock() -> std::sync::MutexGuard<'static, ()> {
        ENV_MUTEX.lock().unwrap_or_else(|poisoned| {
            poisoned.into_inner()
        })
    }
    ...
    fn test_passphrase_key_derivation() {
        ...
        assert_eq!(&*key1, &*key2, "Keys should be identical with same passphrase");
    ...
    fn test_keystore_locked_protection() {
        ...
        assert!(
            ...
            "Should fail with KeystoreLocked when state exists but no key available, got: {:?}", result
        );
    ...
    fn test_salt_file_security() {
        ...
        assert!(overwrite_result.is_err(), "Should not overwrite existing salt");
