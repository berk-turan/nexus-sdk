...
fn validate(dag: Dag) -> Result<(), String> {
    ...
    if !graph.is_directed() || petgraph::algo::is_cyclic_directed(&graph) {
        return Err("The provided graph contains one or more cycles.".to_string());
    }
...
fn has_correct_order_of_actions(graph: &DiGraph<GraphNode, ()>) -> Result<(), String> {
    for node in graph.node_indices() {
        ...
        match vertex {
            ...
            GraphNode::InputPort { .. } if neighbors.len() != 1 => {
                return Err(format!("'{}' must have exactly 1 outgoing edge", vertex))
            }
            ...
            GraphNode::OutputVariant { .. } if neighbors.is_empty() => {
                return Err(format!("'{}' must have at least 1 outgoing edge", vertex))
            }
...
/// graph.
fn try_into_graph(dag: Dag) -> Result<GraphAndVertexEntryGroups, String> {
    ...
    if vertex_entry_groups.is_empty() {
        return Err("The DAG has no entry vertices or ports.".to_string());
    }
    ...
    for vertex in &dag.vertices {
        ...
        if !graph_nodes.contains_key(&vertex_ident) {
            return Err(format!("'{}' is not connected to the DAG.", vertex_ident));
        }

        if !all_vertices.insert(vertex_ident.clone()) {
            return Err(format!("'{}' is defined multiple times.", vertex_ident));
        }
    }
    ...
    for vertex in &dag.vertices {
        ...
        for entry_port in entry_ports {
            ...
            if graph_nodes.contains_key(&entry_port_ident) {
                return Err(format!(
                    "'{}' has an edge leading to it and therefore cannot be an entry port.",
                    entry_port_ident
                ));
            }

            if !all_entry_ports.insert(entry_port_ident.clone()) {
                return Err(format!("'{}' is defined multiple times.", entry_port_ident));
            }
        }
    }
    ...
    for default_value in default_values {
        ...
        if graph_nodes.contains_key(&default_value) || all_entry_ports.contains(&default_value) {
            return Err(format!(
                "'{}' is an entry port or has an edge leading into it and therefore cannot have a default value.",
                default_value
            ));
        }
    }
    ...
    for vertex in &dag.vertices {
        ...
        if has_outputs && has_edges {
            return Err(format!(
                "'{}' cannot have both outgoing edges and ports marked as output.",
                vertex_ident
            ));
        }
    }
