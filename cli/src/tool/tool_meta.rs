...
pub(crate) async fn tool_meta(ident: ToolIdent) -> AnyResult<ToolMeta, NexusCliError> {
    match (ident.off_chain, ident.on_chain) {
        (Some(url), None) => get_off_chain_tool_meta(url).await,
        (None, Some(ident)) => get_on_chain_tool_meta(ident).await,
        _ => unreachable!("Checked by clap"),
    }
}

/// Retreive the metadata for an off-chain tool based on the provided URL.
async fn get_off_chain_tool_meta(url: reqwest::Url) -> AnyResult<ToolMeta, NexusCliError> {
    ...
    // Get meta.
    let meta_handle = loading!("Getting tool meta...");
...
/// Retreive the metadata for an on-chain tool based on the provided identifier.
async fn get_on_chain_tool_meta(_ident: String) -> AnyResult<ToolMeta, NexusCliError> {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_meta_oks_off_chain_tools() {
        tokio::spawn(
